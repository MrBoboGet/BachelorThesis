...
\begin{definition}[Rooted Tree]{\cite{Trees}}
    ...
    and $v \in V$. In this case, $v$ is also called the root of the tree.
\end{definition}
...
color from $used$ at most  $|deg(v)|+1$ steps. This part correspond to lines
5-8.
...
in the partition is the vertex with the highest degree among the unpartition
vertices, we call this set $S$ like in \autoref{alg:rlf}. We can see that the
vertex with the the highest degree is vertex 4. Now remove it from this graph
and add it to the current part, aswell as adding it's neighbour to the set
of adjecent vertices to the current part. We represent vertices in the current
partition with a red color, and vertices that are adjecent to the current part
...
vertices not adjecent to the current part, 1 and 6 in this case, that has the
most highest amount of neighbors that are unassigned and adjecent to the current
part. We can see that 6 has one neighbour that is unassigned and adjecent to the
current part, while 1 has two. We therefor add 1 next and get the graph in
\autoref{fig:rlf3}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \IF{$M$ is parallel or $M$ is series}
        ...
        \ENDFOR
        \IF{$M$ is parallel}
          \STATE $\mathcal{G} \leftarrow \{G(w)\colon w\in\child(M)\}$ 
          \STATE $G^* \leftarrow \argmax_{w\in\child(v)} |\chi(G(w))|$
          \STATE $S \leftarrow \sigma(V(G^*))$ 
          \FOR {$H\in\mathcal{G}\setminus \{G^*\}$} 
             \STATE randomly choose an injective map $\phi:\sigma(H)\to S$
...
\begin{algorithmic}[1]
    ...
    \ELSIF{$M$ is prime}
        \IF{$|M| \leq T$}
            \STATE{Color the graph $G[M]$ with $H$}
            \RETURN
        \ENDIF
...
all disturbed cographs, as well as the performance on DIMACS graphs. The graphs
both display the different heuristics on the x-axis
