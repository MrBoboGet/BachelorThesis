...
\begin{definition}[Cycle]
    A cycle is a    sequence of vertices $(v_1,\cdots,v_i,v_1)$
    if $(v_1,\cdots,v_i)$ is a path.
\end{definition}

A cycle is very similar to a regular path except that we allow, and require,
that the first and last vertex to be the same, and can be seen as a way to walk
in a circle. 
...
\begin{definition}[Rooted tree]{\cite{Trees}}
    ...
    and with a distinguished $v \in V$, called the root of the tree.
\end{definition}
...
with a few but not necessarily optimal number of colors, have to be used.
...
it's children, except for prime vertices.
...
\begin{definition}[Modular decomposition tree]{\cite{MDTree}}
    ...
    M'' \subset M$. Every vertex with associatd module $X$ also has a label
...
\begin{definition}[Child module]
    A module $M'$ is a child module to module $M$ for a graph $G = (V,E)$, if
    the vertex with associated module $M'$ is a child to the vertex with associated
    module $M$ in the modular decomposition tree for $G$.
\end{definition}
...
join on the induced subgraphs of its children's associated modules, and it being
...
module, and that the whole of $\{1,2,3,4\}$ forms another strong module.  This
...
the induced subgraphs of it's submodules, it is series. If none of these are
...
in a deterministic \todo{Deterministic?} fashion, by using the saturation degree.
...
Each vertex is colored so that the vertex with the highest saturation degree is
...
This partition is constructed one part at a time. Creating a partition with the
fewest amount of elements, such that the vertices in the different parts are
non-adjacent  to each other, would be equivalent to finding an optimal
coloring, the heuristic therefore lies in how the vertices are added to the
current part in the partition.
...
a new vertex-color pair is added. If there are more than one coloring that give
...
is, first a coloring is made with RLF, and the number of used colors is
assigned to $k$ and the resulting coloring to $\sigma$.  The then we try to color the graph with TabuCol with $k-1$ allowed colors
colors. If this results in a proper coloring, so is this step repeated and we subtract $k$ by one,
and we save the coloring to $\sigma$.
If the coloring is not proper, so the current $\sigma$ the coloring that is applied.
This procedure can be seen in \autoref{alg:tabu-lowest}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE $\sigma \leftarrow $ empty coloring
        \WHILE{true}
...
\todo{why question mark?}
Otherwise, all of it's children are colored
...
heuristic, and then every child module with the same color in the quotient graph
can now be recolored to use the same set of colors.
...
an amount set so that they can be colored parallel efficiently.
...
Ensuring that roughly the same percent of verices are within a prime module
