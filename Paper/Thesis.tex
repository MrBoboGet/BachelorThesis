...
construction is not straightforwad, having an arbitrary internal structure.
...
\end{definition}
For convenience, we also use $\chi$ to denote the amount of colors used by a
graph for a specific coloring, not neccesarilly meaning the optimal.
...
\begin{definition}[Saturation degree]
    ...
    it's colored neighbours, that is $sat(v) = |\{c  : c = \sigma(u), u \in N_G(v) \}|$.
\end{definition}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE $\sigma \leftarrow \text{random non-proper coloring with $k$ colors}$
        \STATE $CurIt \leftarrow 0$
        \STATE $CurrentClash = \textbf{Clashes}(\sigma)$
...
Something that we can see immediately from the tabels, is that applying a
...
also not negligable, averaging on about 15\%.  This also means that there seems
